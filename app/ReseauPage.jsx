...
function composeFeed(user, posts, platformItems) {
  ...
  for (let pos = 0; pos < TARGET; pos++) {
    ...
    if (cIdx < sel.contacts.length) feed.push(sel.contacts[cIdx++]);else
    if (fIdx < sel.fofs.length) feed.push(sel.fofs[fIdx++]);else
    if (oIdx < sel.others.length) feed.push(sel.others[oIdx++]);else
    if (pIdx < sel.platform.length) feed.push(sel.platform[pIdx++]);
  }
  return feed;
}
...
function PostMenu({ post, onHide }) {
  ...
  const copyLink = () => {
    ...
    if (navigator.clipboard) navigator.clipboard.writeText(url).then(() => window.showToast?.('Lien copié dans le presse-papiers', { type: 'success', icon: '🔗' }));else
