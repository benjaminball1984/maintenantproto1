// =====================================================================================
// stripe-webhook — Pure handler (canonical location, testable depuis vitest)
//
// Ce fichier contient la logique pure du webhook Stripe (types + handle).
// Pas d'import Deno, pas de dépendance externe : c'est le contrat partagé
// entre les tests vitest côté `web/` et le bootstrap Deno côté
// `supabase/functions/stripe-webhook/index.ts` (qui appelle `handle` avec
// les implémentations DI réelles : Stripe SDK, supabase-js admin).
//
// Historique :
//   - étape 19 : extrait initialement dans
//     `supabase/functions/stripe-webhook/handler.ts` pour qu'il soit
//     testable depuis vitest sans tirer les imports Deno-only de
//     `index.ts`.
//   - étape 23 (H2-arch) : déplacé ici dans `web/src/lib/` pour que les
//     tests vitest ne nécessitent plus un import cross-package fragile
//     vers `../../../supabase/functions/...`. Le fichier
//     `supabase/functions/stripe-webhook/handler.ts` est conservé comme
//     thin re-export pour que le bootstrap Deno (`index.ts`) puisse
//     continuer à utiliser `./handler.ts` — le bundler `supabase
//     functions deploy` suit la chaîne d'imports relatifs.
//
// Évènements gérés :
