...
function formatDate(value: string | null): string | null {
  ...
  return new Date(time).toLocaleDateString('fr-FR', { day: '2-digit', month: 'long', year: 'numeric' });
}
...
export default function HousingDetailPage() {
    ...
    return (
      <main style={pageStyle}>
        <div role="alert" style={errorBoxStyle}>
          {postgrestErrorMessage(error) ?? 'Hébergement introuvable.'}
        </div>
        <Link to="/services/housing" style={ctaSecondaryStyle}>
  ...
  const handleShare = async () => {
    ...
    if (navigatorWithShare && 'share' in navigatorWithShare) {
      try {
        await navigatorWithShare.share({ title: housing.title, url });
        setShared(true);
        return;
      } catch {
        // fallback clipboard
  ...
  const fromLabel = formatDate(housing.available_from);
  const toLabel = formatDate(housing.available_to);
  ...
  const isHost = Boolean(user && user.id === housing.host_id);

  return (
    <main style={pageStyle}>
      <Link to="/services/housing" style={backLinkStyle}>
        <IconArrowLeft /> Tous les hébergements
      </Link>
      <header style={heroStyle}>
        ...
        <h1 style={titleStyle}>{housing.title}</h1>
        ...
        <p style={descriptionStyle}>{housing.description}</p>

        <div style={ctaRowStyle}>
          {isHost ? (
            <span
              style={ctaSecondaryStyle}
              aria-label="Vous êtes l’hôte de cette annonce"
            >
              <IconHome /> Vous êtes l’hôte
            </span>
          ) : (
            <Link to={`/services/housing/${housing.id}/request`} style={ctaPrimaryStyle}>
              <IconCalendar /> Faire une demande
            </Link>
          )}
